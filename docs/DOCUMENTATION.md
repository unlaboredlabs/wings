...
# Generate a local config and allow your browser/control-plane origin
wings configure --allowed-origin https://control.example.com
```
...
token_id: <generated-by-configure>
token: <generated-by-configure>
...
# Legacy/default browser origin used by some compatibility paths.
# Prefer `allowed_origins` for new setups.
remote: ""
remote_query:
  timeout: 30 # seconds
...
Generate a local configuration file and local API credentials.
...
| Flag | Description |
|------|-------------|
| `--allowed-origin` | Browser/control-plane origin allowed for CORS and websocket access |
| `--token` | Wings API token; auto-generated if omitted |
| `--token-id` | Wings API token identifier; auto-generated if omitted |
